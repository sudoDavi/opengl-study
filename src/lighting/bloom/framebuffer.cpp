	...
	: AttachmentType{ attachmentType }
{
	...
	CreateBuffer(BufferType::Color, &m_colorBuffer);
	CreateBuffer(BufferType::Color, &m_colorBuffer1);
	AttachBuffer(BufferType::Color, m_colorBuffer, GL_COLOR_ATTACHMENT0);
	AttachBuffer(BufferType::Color, m_colorBuffer1, GL_COLOR_ATTACHMENT1);
...
void Framebuffer::AttachBuffer(BufferType type, uint32_t colorBuffer, GLenum colorAttachment) {
	if (AttachmentType == Framebuffer::AttachTyp::Texture) {
		switch (type) {
			case BufferType::Color :
				glFramebufferTexture2D(GL_FRAMEBUFFER, colorAttachment, GL_TEXTURE_2D, colorBuffer, 0);
				break;
			case BufferType::Depth :
				glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, m_depthBuffer, 0);
				break;
...
void Framebuffer::CreateBuffer(Framebuffer::BufferType type, uint32_t* colorBuffer_p) {
	if ( AttachmentType == Framebuffer::AttachTyp::Texture) {
		switch (type) {
			case BufferType::Color :
				glGenTextures(1, colorBuffer_p);
				glBindTexture(GL_TEXTURE_2D, *colorBuffer_p);
